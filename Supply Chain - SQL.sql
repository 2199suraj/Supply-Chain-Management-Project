...
select case when grouping(cust.Cust_Region) then "Grand Total" else cust.Cust_Region end Cust_Region, 
concat(round(sum((lb.Quantity*lb.Price))/1000000, 2)," ", "Million") as LAB_SALES 
from lab_data lb INNER JOIN customer cust on lb.cust_key=cust.cust_key group by cust.Cust_Region with rollup;

#Store State Wise Sales
...
#Customer State Wise Sales
select case when grouping(cust.Cust_State) then "Grand Total" else cust.Cust_State end Cust_State, 
concat(round(sum((lb.Quantity*lb.Price))/1000000, 2)," ", "Million") as LAB_SALES 
from lab_data lb INNER JOIN customer cust on lb.cust_key=cust.cust_key group by cust.Cust_State with rollup;
...
Select sum(Quantity_on_Hand*Price) as "In_Hand_Inventory_Value" from inventory;
